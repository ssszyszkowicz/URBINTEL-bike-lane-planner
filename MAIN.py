...
if __name__ == '__main__':
    ...
    if 0: # LTS-motor_allowed-best_infra analysis:
        T = X['NWK'].TRs
        ma = T['motor_allowed']
        fL = T['fwd_LTS']
        bL = T['bwd_LTS']
        fbi = T['fwd_best_infra']
        bbi = T['bwd_best_infra']
        S = [str(fbi[n])+str(l) for n,l in enumerate(fL)]
        S += [str(bbi[n])+str(l) for n,l in enumerate(bL)]
        histo(S)


    if 0: # analysis of OSM building types (Vancouver BC):
        od = X['OSM_DAT']
        wde = od.props_decoder
        wpb = od.way_props['building']
        BT = []
        BDK = []
        AMENITY = []
        OFBT = []
        for b in wpb:
            bd = wde(b)
            ofbt = osmfilter.building_type(bd)
            OFBT.append(ofbt)
            bt = bd.get('building',bd.get('building:part','no'))
            BT.append(bt)
            if bt=='yes':
                bdk = list(bd.keys())
                BDK += bdk
                if 'amenity' in bd: AMENITY.append(bd['amenity'])
        if 0:
            print('building(:part) types histogram:')
            histo(BT)
            print('building(:part) properties histogram:')
            histo(BDK)
            print('building(:part) "amenity" histogram:')
            histo(AMENITY)
        if 1:
            histo(OFBT)
    ...
    if 0:
        ...
        class ZZ:
            def __init__(self, TRs):
                self.OA = TRs.get_osm_attribute
                self.E = TRs.extra
            def __getitem__(self, i):
                if i in self.E: return self.E[i]
                else: return self.OA(i)
        class AA:
            def __init__(self, GLOBAL):
                self.OR = GLOBAL['NWK'].TRs.osm_road
                self.P = GLOBAL['MAP'].roads.props
                self.LTS = GLOBAL['NWK'].TRs['vis_LTS'] 
            def __call__(self, B):
                n = B.nonzero()[0]
                print(len(n),'true out of',len(B))
                for i in Qr(min(100, len(n))):
                    print('vis_LTS =',self.LTS[n[i]])
                    print(self.P[self.OR[n[i]]].replace('\x00','~'))
        Z = ZZ(TRs)
        A = AA(X)
        A((Z['fwd_LTS']==2) & (Z['fwd_force_dismount']==1)) #!
        
        
        

    
    ###globals().update(Q(X,'X'))
    #T = X['TEMP']
    #write_shp_from_dict(X['NWK'].export_to_shp(Q(X,'MAP centre')),'test.shp')
    #B = Q(X,"MAP optional bikeways optional streetname")
    #T = Q(X,"MAP roads property streetName")
    #########
    if 0:
        main_buildings(X)
        Q>X
        RB=(Q("MAP optional Rbuildings")).xy()
        print(len(RB), sum([len(v[0]) for v in RB]))
        j = QeN(RB, lambda a: triangulate_contour(a[0], True), 1000)
        #k = QeN(RB, lambda a: triangulate_contour(a[0], False), 1000)
        z = RB[21250:21260]
    #QcF(main_buildings,[X])
